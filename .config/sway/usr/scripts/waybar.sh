...
waybar -c "${USER_CONFIG:-"$HOME/.config/sway/usr/templates/waybar/config.jsonc"}" -s "${USER_STYLE:-"$HOME/.config/sway/usr/templates/waybar/style.css"}" > $(mktemp -t XXXX.waybar.log)
